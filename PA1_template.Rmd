...
```{r echo=TRUE}
activity <- read.csv("~/play/RepData_PeerAssessment1/activity.csv", 
...
ggplot(steps_by_date, aes(x=date, y=steps)) + geom_histogram(stat="identity") +
  ...
  ggtitle("Histogram of Steps by Date")
```
...
For this data set, the mean number of steps taken per day is `r mean_steps` and
...
ggplot(steps_by_interval, aes(x=interval, y=steps)) + geom_line() + 
  xlab("5-minute interval") + 
  ylab("Average number of steps taken, averaged across all days") + 
  ggtitle("Time Series plot of Steps Taken by Average Interval")
...
interval number `r max_interval`.
...
There are `r missing_record_count` records with an NA. This is 
`r missing_record_count/nrow(activity)*100`% of the data set. So it's a not
...
interval_indices <- match(activity[! complete.cases(activity),]$interval, 
...
ggplot(steps_by_date_imputed, aes(x=date, y=steps)) + geom_histogram(stat="identity") +
  ylab("Total Number of Steps") + xlab("Date") + 
  ggtitle("Histogram of Steps by Date (Imputed)")
```
...
For this data set, the mean number of steps taken per day is `r mean_steps_imputed` 
and the median is `r median_steps_imputed`. Due to the fact that the aggregate 
...
steps_by_interval_weekday <- aggregate(steps ~ interval, 
                                       subset(activity.imputed, 
                                              day_type == "weekday"),
                                       mean)

#aggregate steps for weekends
steps_by_interval_weekend <- aggregate(steps ~ interval, 
                                       subset(activity.imputed, 
                                              day_type == "weekend"),
                                       mean)
...
ggplot(steps_by_interval_combined, aes(x=interval, y=steps)) + geom_line() + 
  xlab("5-minute interval") + 
  ylab("Average number of steps taken, averaged across all like days") + 
  ggtitle("Time Series plot of Steps Taken by Average Interval") + facet_wrap(~ day_type, ncol=1)
```
